<?php
class Rpaymentjournal extends CI_Controller {
	...
	public function result(){
		...
		$sql = "
			select 
				...
				ifnull(v_bkpf.name2,'') as name2,
				ifnull(v_bven.saknr,'') as saknr,
				ifnull(tbl_glno.sgtxt,'') as sgtxt,
				ifnull(v_bven.debit,'') as debit,
				ifnull(v_bven.credi,'') as credi,
				ifnull(v_bven.statu,'') as statu,
			...
			where 
				v_bkpf.bldat BETWEEN '{$_POST['start_date']}' and '{$_POST['end_date']}' and v_bven.belnr like 'PV%' {$search}
	...
	public function pdf(){
		...
		$report = $client->runReport('/ai_account/rpaymentjournal', 'pdf', null, $controls);
	...
	public function excel(){
		...
		$report = $client->runReport('/ai_account/rpaymentjournal', 'xlsx', null, $controls);
