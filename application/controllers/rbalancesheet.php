<?php
class RBalanceSheet extends CI_Controller {
	...
	public function pdf(){
		...
		$controls = array('start_date' => intval(mktime(0,0,0,intval($sd[1]),intval($sd[2]),intval($sd[0])))*1000,
						  ...
						  'comid' => $_GET["comid"]);
		$report = $client->runReport('/ai_account/rbalancesheet', 'pdf', null, $controls);
	...
	public function excel(){
		...
		$controls = array('start_date' => intval(mktime(0,0,0,intval($sd[1]),intval($sd[2]),intval($sd[0])))*1000,
						  'end_date' => intval(mktime(0,0,0,intval($ed[1]),intval($ed[2]),intval($ed[0])))*1000,
						  'comid' => $_GET["comid"]);
		$report = $client->runReport('/ai_account/rbalancesheet', 'xlsx', null, $controls);
