...
class Otherexpense extends CI_Controller {
	...
	function index()
	{
		...
		$objPHPExcel->getProperties()->setCreator("Prime BizNet")
									 ...
									 ->setTitle("Expnese")
									 ->setSubject("Expnese")
									 ->setDescription("Expnese information.");
		...
		// add header data
		$current_sheet
	            ->setCellValue('A1', 'Expnese No')
	            ->setCellValue('B1', 'Expnese Date')
	            ->setCellValue('C1', 'Vendor No')
	            ->setCellValue('D1', 'Vendor Name')
	            ->setCellValue('E1', 'Project No')
				->setCellValue('F1', 'Tax Invoice')
		...
		for($i=0;$i<count($result_array);$i++){
			...
			$current_sheet
					...
					->setCellValue('G'.$excel_i, $value['refno'])
		...
		header('Content-Disposition: attachment;filename="expense_'.date('Y-m-d_H:i:s').'.xls"');
