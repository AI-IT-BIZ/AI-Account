...
class Pettyreim extends CI_Controller {
	...
	function index()
	{
		...
		$objPHPExcel->getProperties()->setCreator("Prime BizNet")
									 ...
									 ->setTitle("CPV Petty Cash")
									 ->setSubject("CPV Petty Cash")
									 ->setDescription("CPV Petty Cash information.");
		...
		// add header data
		$current_sheet
	            ->setCellValue('A1', 'CPV  No')
	            ->setCellValue('B1', 'CPV  Date')
	            ->setCellValue('C1', 'Vendor No')
	            ->setCellValue('D1', 'Vendor Name')
				->setCellValue('E1', 'Limit Amount')
	            ->setCellValue('F1', 'Limit Remain Amt')
	            ->setCellValue('G1', 'CPV Amount')
				->setCellValue('H1', 'CPV Amt Remain')
		...
		for($i=0;$i<count($result_array);$i++){
			...
			$excel_i = $i+2;
			$current_sheet
					->setCellValue('A'.$excel_i, $value['remnr'])
					...
					->setCellValue('E'.$excel_i, number_format($value['deamt'],2,'.',','))
		            ->setCellValue('F'.$excel_i, number_format($value['reman'],2,'.',','))
		            ->setCellValue('G'.$excel_i, number_format($value['dispc'],2,'.',','))
		            ->setCellValue('H'.$excel_i, number_format($value['upamt'],2,'.',','))
		...
		header('Content-Disposition: attachment;filename="petty_cash_'.date('Y-m-d_H:i:s').'.xls"');
