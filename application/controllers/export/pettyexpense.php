...
class Pettyexpense extends CI_Controller {
	...
	function index()
	{
		...
		$objPHPExcel->getProperties()->setCreator("Prime BizNet")
									 ...
									 ->setTitle("Petty Cash")
									 ->setSubject("Petty Cash")
									 ->setDescription("Petty Cash information.");
		...
		// add header data
		$current_sheet
	            ->setCellValue('A1', 'Petty Cash No')
	            ->setCellValue('B1', 'Petty Cash Date')
	            ->setCellValue('C1', 'Vendor No')
	            ->setCellValue('D1', 'Vendor Name')
				->setCellValue('E1', 'CPV No')
	            ->setCellValue('F1', 'PC Net Amount')
		...
		for($i=0;$i<count($result_array);$i++){
			...
			$current_sheet
					...
					->setCellValue('E'.$excel_i, $value['remnr'])
		            ->setCellValue('F'.$excel_i, number_format($value['deamt'],2,'.',','))
		            ->setCellValue('G'.$excel_i, number_format($value['netwr'],2,'.',','))
		            ->setCellValue('H'.$excel_i, number_format($value['dispc'],2,'.',','))
