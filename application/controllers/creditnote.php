...
// End for report
	...
	function save_cns(){
		...
		if (!empty($query) && $query->num_rows() > 0){
			...
			$this->db->update('vbcn', $formData);
		}else{
			$id = $this->code_model->generate('CN', 
			$this->input->post('bldat'));
			$this->db->set('crenr', $id);
			//$this->db->set('erdat', 'NOW()', false);
			db_helper_set_now($this, 'erdat');
		    $this->db->set('ernam', $current_username);
			$this->db->insert('vbcn', $formData);
...
// Save GL Posting	
		...
		if ($this->db->trans_status() === FALSE){
			...
			try{
				...
				if(!empty($inserted_id)){
					$q_row = $this->db->get_where('vbcn', array('crenr'=>$inserted_id));
					$row = $q_row->first_row();
					...
					if($status_changed){
						$q_row = $this->db->get_where('vbcn', array('crenr'=>$post_id));
						$row = $q_row->first_row();
    ...
    function save_cnp(){
		...
		if (!empty($query) && $query->num_rows() > 0){
			...
			$id = $this->code_model->generate('PN', 
			$this->input->post('bldat'));
			$this->db->set('crenr', $id);
			//$this->db->set('erdat', 'NOW()', false);
			db_helper_set_now($this, 'erdat');
    ...
    if($this->input->post('statu') == '02'){
		...
		if ($this->db->trans_status() === FALSE){
			...
			try{
				...
				// send notification email
				if(!empty($inserted_id)){
					$q_row = $this->db->get_where('ebcn', array('crenr'=>$inserted_id));
					$row = $q_row->first_row();
					$this->email_service->sendmail_create(
						'PN', 'Purchase Credit Note',
						$inserted_id, $total_amount,
						$row->ernam
					);
				}else if(!empty($post_id)){
					if($status_changed){
						$q_row = $this->db->get_where('ebcn', array('crenr'=>$post_id));
						$row = $q_row->first_row();
						$this->email_service->sendmail_change_status(
							'PN', 'Purchase Credit Note',
							$post_id, $total_amount, $row->statu,
   ...
   public function loads_percombo(){
	...
	function loads_items(){
		...
		if(empty($invnr) && $invnr!=0){
			$iv_id = $this->input->get('crenr');
			$q_qt = $this->db->get_where('vbcn', array(
				'crenr'=>$iv_id
			));
	...
	function loads_itemp(){
		...
		if(empty($invnr) && $invnr!=0){
			$iv_id = $this->input->get('crenr');
			$q_qt = $this->db->get_where('ebcn', array(
				'crenr'=>$iv_id
			));
	...
	function loads_gl_items(){
		...
		if($iv_id!=0){
		   ...
		   $kunnr = $this->input->get('kunnr');  //Vendor Code
