...
class Material extends CI_Controller {
	...
	function read()
	{
		...
		foreach($result AS $value){
			array_push($materials, $value['matnr']);
		}
		$this->db->select('matnr');
		$this->db->where_in('matnr', $materials);
		$query = $this->db->get('mara');
		$exist_materials = $query->result_array();
		$result = check_exist_pk($exist_materials, $result, 'matnr', 'Primary key is duplicate');

		// check valid Material Group
		$material_group = array();
		foreach($result AS $value){
			array_push($material_group, $value['matkl']);
		}
		$this->db->select('matkl');
		$this->db->where_in('matkl', $material_group);
		$query = $this->db->get('mgrp');
		$valid_material_group = $query->result_array();
		$result = check_exist($valid_material_group, $result, 'matkl', 'Material Group is not exist');

		// check valid Material type
		$material_type = array();
		foreach($result AS $value){
			array_push($material_type, $value['mtart']);
		}
		$this->db->select('mtart');
		$this->db->where_in('mtart', $material_type);
		$query = $this->db->get('mtyp');
		$valid_material_type = $query->result_array();
		$result = check_exist($valid_material_type, $result, 'mtart', 'Material Type is not exist');
