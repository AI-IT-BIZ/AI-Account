Ext.define('Account.Payment.MainWindow', {
	...
	initComponent : function() {
		...
		this.addAct = new Ext.Action({
			...
			disabled: !UMS.CAN.CREATE('PY')
		});
		this.editAct = new Ext.Action({
			text: 'Edit',
			iconCls: 'b-small-pencil',
			disabled: !(UMS.CAN.DISPLAY('PY') || UMS.CAN.CREATE('PY') || UMS.CAN.EDIT('PY'))
		});
		this.displayAct = new Ext.Action({
			text: 'Display',
			iconCls: 'b-small-search',
			disabled: !UMS.CAN.DISPLAY('PY')
		});
		this.deleteAct = new Ext.Action({
			text: 'Delete',
			iconCls: 'b-small-minus',
			disabled: !UMS.CAN.DELETE('PY')
		});
        this.excelAct = new Ext.Action({
			text: 'Excel',
			iconCls: 'b-small-excel',
			disabled: !UMS.CAN.EXPORT('PY')
		});
		...
		this.grid = Ext.create('Account.Payment.Grid', {
			region:'center',
			border: false,
			tbar: [this.addAct, this.editAct, this.displayAct, this.deleteAct, this.excelAct,this.importAct]
		});
		
		//this.searchForm = Ext.create('Account.Payment.FormSearch', {
		//	region: 'north',
		//	height:100
		//});
		...
		this.searchForm = Ext.create('Account.Payment.FormSearch', searchOptions);
		...
		this.itemDialog.form.on('afterSave', function(form, action){
			...
			Ext.Msg.alert('Status', 'Save Payment number: '+resultId+' successfully.');
		});
	    ...
	    this.excelAct.setHandler(function(){
			...
			window.location = __site_url+'export/payment/index?'+query;
		});
		// --- after ---
		this.grid.load();
